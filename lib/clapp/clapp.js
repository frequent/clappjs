...
(function (document, RSVP) {
  ...
  function returnModule(name) {
    ...
    return {
      "name": name,
      "callback": clappjs.callback_dict[name] || window[name] ||
          window[name.toUpperCase()] || window.module[name] ||
   ...
   */
  function shimDefine() {
    var args, list, sync, name, index, deps;
    ...
    // if defined and clapp is idle, pass through calls to original define
    ...
    // NOTE: anonymous modules are impossible to correctly associate when
    // multiple anonymous modules should be loaded. The modules just return
    // a callback and there is no way of associating unless RSVP.Queue is used
    // which goes against the whole async idea...
  ...
  declare = function (name, dependency_list, callback) {
    ...
    clappjs.callback_dict[name] = clappjs.callback_dict[name] ||
      new RSVP.Promise(function (resolve) {
        resolver = resolve;
        if (dependency_list.length === 0) {
          return resolve(callback.apply(window));
        }
  ...
  request = function (module_list) {
    ...
    function requestBundleList(my_loading_order) {
      ...
      return RSVP.all(promise_index).fail(console.log);
    }
