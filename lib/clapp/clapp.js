...
(function (document, RSVP) {
  ...
  function setLoadingOrder(module_list, bundle_list) {
    var mod, len, dependency_list, i, position, base;
    ...
    for (i = 0, len = module_list.length; i < len; i += 1) {
      ...
      base = bundle_list[0];
      mod = module_list[i];
      dependency_list = clappjs.dependency_dict[mod];
  ...
  request = function (module_list) {
    ...
    function requestBundles(my_loading_order) {
    ...
    return requestBundles(setLoadingOrder(module_list))
